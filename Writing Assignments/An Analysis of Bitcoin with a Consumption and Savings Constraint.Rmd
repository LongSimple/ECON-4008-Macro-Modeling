...
**Fix the abstract- include prelim results**
...
During the examined period, the returns of Bitcoin ranged from -97.119% to 39,663.05%, and the mean return during this holding period was 238.228%. With 90% confidence, we conclude that the correlation coefficients of the autoregressive regression are statistically significant. 
...
##The Tauchen Algorithm and The Bootstrap Model

We approach the simulation of Bitcoin returns in two ways: The Tauchen Algorithm and The Bootstrap Method. The employment of the Tauchen Algorithm provides discretized aggregate shock states accompanied by a transition probability matrix for each state. We allow a total number of 60 exogenous varying shocks to occur with a maximum loss of 99.88% and a maximum gain of 1,004.9%. The coefficient of relative risk aversion and the subjective discount factor are among the most important parameters in a risky intertemporal environment. We assume the agent has a subjective discount value of $\beta = .85$ and a risk aversion parameter $\gamma = 2$. To examine the shock process of Bitcoin, we adjust the persistence, $\rho$, the mean, $\mu$, and the variance of the error terms, $\sigma_e$, of the algorithm and simulate the returns of Bitcoin over 819 periods. This model fails to accurately simulate returns with summary statistics equal to those observed in the data. Calibrating the model so that $\mu = -6.5, \rho = -.98,$ and $\sigma_e = .0085$ provides the summary statistics seen below. However, variations of the aforementioned parameters in the model do not provide summary statistics that converge to those observed in the data. Furthermore, averaging the summary statistics of multiple simulations does not result in the convergence of summary statistics.  

The Bootstrap Method defines $X_1, X_2, ..., X_n$ as independent, indentically distributed random variables with the distribution function $F$. Suppose that $R(X,F)$ is a random variable of both the observed returns Bitcoin and the distribution $F$. The Bootstrap method estimates the sampling distribution of $R(X, F)$ on the basis of observed returns. We employ the Monte Carlo approximation of the bootstrap distribution. In turn, repeated realizations of returns, $X^{*^i}$, are generated by taking random samples of size $n$ from $\hat{F}_n$. Furthermore, the histogram of corresponding values $F(X^{*^i}, \hat{F}_n)$ are taken as an approximation of the actual bootstrap distribution (Fuh, 1989). To utilize the Bootstrap Method, we adjust collected returns of Bitcoin and discretize the data with 102 equally spaced bins. Thereafter, we fit the bootstrap method with 1,000 boots. The results of our simulations closely align with the observed data. To assess the fit of each model, we include 100 replicates of the Tauchen Algorith and the Bootstrap Model. Subsequently, we average the summary statistics of each model and compare them to the summary statistics of the observed data. Repeated simulations of the Bootstrap Model replicate returns with summary statistics similar to those observed in the data. The results of a single simulation are displayed below. Averaging the summary statistics of multiple simulations of this model results in values that approach those observed in the data. Hence, the Bootstrap Model provides a more accurate representation of the behavior of Bitcoin relative to the Tauchen Algorithm.    
...
The results of our simulations do not provide sufficient evidence to conclude that an increase in the risk aversion parameter results in a decrease in an individual's investment in Bitcoin. We observe that increase the risk aversion parameter from $.5$ to $2$ results in a decrease in the amount of Bitcoin held by the investor and co-occurs with an increase in the agent's investment in Bitcoin. This suggests that the individual's choice is not to solely decrease the amount of Bitcoin he holds. Rather, his choice is to minimize losses by diversifying his portfolio. However, in both the Tauchen Algorithm simulations as well as the Bootstrap Method simulations, an increase in the risk aversion parameter from $2$ to $3$, results in an increase in the the agent's investment in Bitcoin. This decision choice starkly contrasts the decision choice observed when the risk aversion parameter increases from $.5$ to $2$. We also find that the the agent's investment in Treasury Bonds vary based on the model employed. Ergo, we can not strictly conclude that an increase in the risk aversion parameter decreases an individual's investment in a risky asset. 

#Discussion and Conclusion

The results of this study suggest that individuals' investment choices vary based on the risk aversion parameter and exogenous shocks to returns. Although the Tauchen Algorithm and the Bootstrap Model possess the ability to describe the behavior of cryptocurrency, the Bootstrap Model permits relatively more accurate replications. The discrepencies between the returns of each model are attributable to the Bootstrap Model's inclusion of random samples from the observed data. 

It is atypical that an increase in the agent's risk aversion does not result in a decreased investment in risky assets along with an increased investment in risk-less assets. These findings suggest there exists a threshold for investments in risk-free bonds as the risk aversion parameter increases. If the decision rule assumes a concave shape due to an investment threshold, there may be further implications for the agent's investment decisions. Both the Tauchen Algorithm and Bootstrap Model include market shocks that significantly vary throughout each simulation. In turn, the observed decision rule may be determined by the effect exogenous shocks have on an individual's investment decision in addition to adjustments made to the risk aversion parameter. 
